    ...
    for address in cluster.members:
        ...
        ranges.append(
            FunctionRange(
                ...
                origin_file=routine.source_file,
                origin_line=routine.source_line,
            )
        )
        c_line = c_end + 2
        asm_line = asm_end + 2
...
def _write_metadata(context: ExportContext) -> None:
    ...
    def write_header() -> None:
        types_header_name: str | None = None
        if analysis.types:
            stem = clean_path_component(analysis.binary.path.stem)
            types_header_name = f"{stem}.types.h"
            write_text_atomic(
                _need(context.include_dir) / types_header_name,
                build_types_header(analysis),
            )
        write_text_atomic(
            header,
            build_header(
                analysis, context.prototypes, names, types_header=types_header_name
            ),
        )
    ...
    steps: list[tuple[str, Any]] = [
        ...
        (
            "types.json",
            lambda: write_json(root / "types.json", types_json(analysis)),
        ),
...
def build_header(
    ...
    *,
    ...
    lines = [
        ...
        "#include <stdint.h>",
        "",
        # Pseudo-types the decompiler output still references; the real recovered
        # types live in the included <bin>.types.h (when debug info was present).
    ...
    if types_header:
        lines.extend([f'#include "{types_header}"', ""])
    lines.extend(
        [
            "#ifdef __cplusplus",
            'extern "C" {',
...
def build_types_header(analysis: ProgramAnalysis) -> str:
    """Emit the types the backend recovered as real C declarations."""
    lines = [
        "#pragma once",
        "",
        "#include <stdbool.h>",
        ...
        "#include <stdint.h>",
        "",
        "/* Types recovered by ToCode from the binary's debug info / type library. */",
        "",
    ]
    # Order so that aggregate definitions a typedef may reference come first,
    # while keeping each kind grouped for readability.
    order = {"base": 0, "struct": 1, "union": 1, "enum": 1, "typedef": 2}
    for item in sorted(analysis.types, key=lambda t: (order.get(t.kind, 3), t.name)):
        lines.append(
            f"/* {item.kind} {item.name}"
            + (f" (size {item.size})" if item.size is not None else "")
            + " */"
        )
        lines.append(item.c_decl.rstrip())
        lines.append("")
    return "\n".join(lines)
            ...
            "functions": [
                {
                    ...
                    "origin": {
                        "file": item.origin_file,
                        "line": item.origin_line,
                    }
                    if item.origin_file
                    else None,
                }
...
def write_manifest(context: ExportContext) -> Path:
    ...
    write_json(
        path,
        {
            ...
            "types": str((root / "types.json").resolve()),
            "type_count": len(analysis.types),
...
def build_export_agents(
    ...
    lines = [
        ...
        "- `types.json`: catalog of structs/unions/enums/typedefs recovered from the binary's debug info or type library (with C declarations).",
        "- `include/*.types.h`: those recovered types as compilable C, included by the main header (present only when types were recovered).",
        ...
        "- `functions.json`: per-function caller/callee relationships, prototypes, recovered return/param/local types, original source decl file/line (when debug info exists), source ranges, and ASM ranges.",
...
def _build_clusters(
    ...
    # When debug info gives functions a real source file, mirror the original
    # source tree (folder + filename); everything without that info falls back
    # to the call-graph/segment clustering below.
    source_clusters, remaining = _source_file_clusters(analysis)
    if len(remaining) >= FAST_CLUSTER_FUNCTIONS:
        clusters = _fast_clusters(analysis, remaining)
    else:
    ...
    return (
        source_clusters
        + _normalize_clusters(analysis, clusters)
        + _support_clusters(analysis)
    )


def _source_file_clusters(
    analysis: ProgramAnalysis,
) -> tuple[list[Cluster], set[int]]:
    """Group app routines by their DWARF/debug source file.

    Returns the source-grouped clusters and the set of addresses that had no
        ...
        for index, chunk in enumerate(_chunks(members, MAX_FUNCTIONS_PER_FILE)):
            clusters.append(
                Cluster(
                    root=chunk[0],
                    label=source_file,
                    summary=f"Source file {source_file}",
                    members=list(chunk),
                    folder=folder,
                    file_base=f"{base}_{index}" if many else base,
                )
            )
...
def _fast_clusters(
    analysis: ProgramAnalysis, addresses: set[int] | None = None
) -> list[Cluster]:
    by_segment: dict[str, list[int]] = {}
    for routine in analysis.app_routines():
        if addresses is not None and routine.address not in addresses:
            continue
...
def _function_metadata(
    *,
    ...
    if routine.source_file:
        origin = routine.source_file
        if routine.source_line is not None:
            origin = f"{origin}:{routine.source_line}"
...
def _cluster_path(root: Path, cluster: Cluster, file_name: str) -> Path:
    # A source-grouped cluster's folder may be a nested path (e.g. "net/ssl");
    # preserve the directory structure instead of collapsing it. Non-source
    # clusters use a single component ("app", "support", ...) as before.
    folder = cluster.folder if cluster.folder is not None else "app"
    target = root
    for part in folder.replace("\\", "/").split("/"):
        if part and part != ".":
            target = target / clean_path_component(part)
